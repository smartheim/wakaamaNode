...
void lwm2m_data_encode_int(int64_t value,
                           lwm2m_data_t * dataP)
{
    LOG_DATA_ARG("value: %" PRId64 "", value);
                          ...
                          int64_t * valueP)
{
    ...
    LOG_DATA_ARG("result: %d, value: %" PRId64, result, *valueP);
                            ...
                            double * valueP)
{
    ...
    LOG_DATA_ARG("result: %d, value: %f", result, *valueP);
...
void lwm2m_data_encode_bool(bool value,
                            lwm2m_data_t * dataP)
{
    LOG_DATA_ARG("value: %s", value?"true":"false");
                           ...
                           bool * valueP)
{
    ...
    LOG_DATA_ARG("result: %d, value: %s", result, *valueP ? "true" : "false");
...
void lwm2m_data_encode_objlink(uint16_t objectId,
                           ...
                           lwm2m_data_t * dataP)
{
    LOG_DATA_ARG("value: %d/%d", objectId, objectInstanceId);
                     ...
                     lwm2m_data_t ** dataP)
{
    ...
    LOG_DATA_ARG("format: %s, bufferLen: %d", STR_MEDIA_TYPE(format), bufferLen);
    LOG_URI(uriP);
    switch (format)
    {
...
{
    LOG_URI(uriP);
    LOG_DATA_ARG("size: %d, formatP: %s", size, STR_MEDIA_TYPE(*formatP));
    ...
    if (*formatP == LWM2M_CONTENT_OPAQUE
     && dataP->type != LWM2M_TYPE_OPAQUE)
    {
        LOG_DATA("Opaque format is reserved to opaque resources.");
        return -1;
    }

    LOG_DATA_ARG("Final format: %s", STR_MEDIA_TYPE(*formatP));
