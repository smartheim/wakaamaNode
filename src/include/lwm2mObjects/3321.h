...
// Custom, overrideable types for Opaque and String resources
    ...
    #ifndef ResetMinandMaxMeasuredValues3321
    class ResetMinandMaxMeasuredValuesType : public Opaque<30> {};
    #endif
    
    #ifndef CurrentCalibration3321
    class CurrentCalibrationType : public PreallocString<30> {};
    #endif
    
    #ifndef ApplicationType3321
    class ApplicationTypeType : public PreallocString<30> {};
    #endif
    

/* \brief Class for object 3321 - Altitude
 *  This IPSO object should be used with an altitude sensor to report altitude above sea
 *  level in meters. Note that Altitude can be calculated from the measured pressure given
 *  the local sea level pressure. It also provides resources for minimum and maximum measured
 *  values, as well as the minimum and maximum range that can be measured by the sensor. An
 *  example measurement unit is meters (ucum: m).
 */
 ...
 *  example measurement unit is meters (ucum: m).
 */
class object : public Lwm2mObject<3321, object, instance> {
public:
...
inline bool operator== (KnownObjects::id3321::RESID c1, uint16_t c2) { return (uint16_t) c1 == c2; }
inline bool operator== (uint16_t c2, KnownObjects::id3321::RESID c1) { return (uint16_t) c1 == c2; }
