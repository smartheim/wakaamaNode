                        ...
                        const char * id)
{
    ...
    {
        length = lwm2m_data_serialize((uriStr != nullptr) ? &uri : nullptr, size, tlvP, &format, &buffer);
        if (length <= 0)
        {
                        ...
                        size_t original_length)
{
    ...
    {
        ASSERT_EQ(original_length, length);

        if ((original_length != (size_t)length) ||
            (memcmp(original_buffer, buffer, (size_t)length) != 0))
        {
            std::stringstream ss;
            for(int i=0; i<length; ++i)
                ss << "Comparing buffer after parse/serialize failed for "<<std::hex << (int)buffer[i];
            std::cout <<ss.str()<<std::endl;
            ss.clear();
            for(int i=0; i<length; ++i)
                ss << std::hex << (int)buffer[i];
            std::cout <<"Instead: "<<ss.str()<<std::endl;
            FAIL();
        }
    }
                     ...
                     const char * id)
{
    ...
    size = lwm2m_data_parse((uriStr != nullptr) ? &uri : nullptr, testBuf, testLen, format, &tlvP);
    if (size < 0)
    {
                              ...
                              const char * id)
{
    ...
    // Serialize to the same format and compare to the input buffer
    test_data_and_compare(uriStr, format, tlvP, size, id, (const uint8_t*)expectBuf, expectLen);
...
TEST(tlv_json, test_1)
{
    uint8_t buffer[] = {0x03, 0x0A, 0xC1, 0x01, 0x14, 0x03, 0x0B, 0xC1, 0x01, 0x15, 0x03, 0x0C, 0xC1, 0x01, 0x16};
    size_t testLen = sizeof(buffer);
    lwm2m_media_type_t format = LWM2M_CONTENT_TLV;
    lwm2m_data_t * tlvP;
    int size = lwm2m_data_parse(nullptr, buffer, testLen, format, &tlvP);
    ASSERT_GT(size,0);
    // Serialize to the same format and compare to the input buffer
    test_data_and_compare(nullptr, format, tlvP, size, "1", (const uint8_t*)buffer, testLen);

}

TEST(tlv_json, test_2)
{
    const uint8_t buffer[] = {
        ...
        0x55};
    test_raw(nullptr, buffer, sizeof(buffer), LWM2M_CONTENT_TLV, "2");
}

TEST(tlv_json, test_3)
{
    const char buffer[] = "{\"e\":[                                              \
    ...
    test_raw("/3/0", (const uint8_t *)buffer, sizeof(buffer)-1, LWM2M_CONTENT_JSON, "3");
}

TEST(tlv_json, test_4)
{
    const char * buffer = "{\"e\":[                                      \
                         {\"n\":\"0\",\"sv\":\"a \\\"test\\\"\"},   \
                         {\"n\":\"1\",\"v\":2015},                  \
    ...
    test_raw("/11/3", (const uint8_t *)buffer, strlen(buffer), LWM2M_CONTENT_JSON, "4");
}

TEST(tlv_json, test_5)
{
    ...
    test_raw_expected("/65", (const uint8_t *)buffer, sizeof(buffer)-1, expect, sizeof(expect)-1, LWM2M_CONTENT_JSON, "5");
}

TEST(tlv_json, test_6)
{
    ...
    test_raw(nullptr, (const uint8_t *)buffer, strlen(buffer), LWM2M_CONTENT_JSON, "6");
}

TEST(tlv_json, test_7)
{
    const char buffer[] = "{\"e\":[                              \
                         {\"n\":\"1\",\"v\":1234},          \
                         {\"n\":\"3\",\"v\":56.789},        \
    ...
    test_raw_expected(nullptr, (const uint8_t *)buffer, sizeof(buffer)-1, expect, sizeof(expect)-1, LWM2M_CONTENT_JSON, "7a");
    test_raw_expected("/12", (const uint8_t *)buffer, sizeof(buffer)-1, expect, sizeof(expect)-1, LWM2M_CONTENT_JSON, "7b");
    test_raw_expected("/12/0", (const uint8_t *)buffer, sizeof(buffer)-1, expect, sizeof(expect)-1, LWM2M_CONTENT_JSON, "7c");
    test_raw("/12/0/3", (const uint8_t *)buffer, strlen(buffer), LWM2M_CONTENT_JSON, "7d");
}

TEST(tlv_json, test_8)
{
    ...
    test_raw(nullptr, (const uint8_t *)buffer, sizeof(buffer)-1, LWM2M_CONTENT_JSON, "8");
}

TEST(tlv_json, test_9)
{
    // TODO : Object links are not supported, comment out tests for now
    const char buffer[] = "{ \"bn\":\"/\","
                          "\"e\":["
//                          "{\"n\":\"34/0/0/0\",\"ov\":\"66:0\"},"
//                          "{\"n\":\"34/0/0/1\",\"ov\":\"66:1\"},"
...
//                          "{\"n\":\"66/1/2\",\"ov\":\"FFFF:FFFF\"},"
    ...
    test_raw(nullptr, (const uint8_t *)buffer, sizeof(buffer)-1, LWM2M_CONTENT_JSON, "9");
}
TEST(tlv_json, test_10)
{
    const int dsize = 17;
    lwm2m_data_t * data1 = lwm2m_data_new(dsize);
    uint16_t i;

    for (i = 0; i < dsize; i++) data1[i].id = i;
    ...
    double v = INT64_MAX/10;
    lwm2m_data_encode_float(v, data1 + 13);
    v = INT64_MIN/10;
    lwm2m_data_encode_float(v, data1 + 14);
    lwm2m_data_encode_bool(true, data1 + 15);
    lwm2m_data_encode_bool(false, data1 + 16);

    test_data(nullptr, LWM2M_CONTENT_TLV, data1, dsize, "10a");
    test_data("/12/0", LWM2M_CONTENT_JSON, data1, dsize, "10b");
}
